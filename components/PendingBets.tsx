...
function ResultLegend({ filter }: { filter?: LegResult[] }) {
  ...
  return (
    <div style={{ padding: '8px 12px 4px', display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(210px, 1fr))', gap: '5px 16px', borderTop: '1px solid rgba(255,255,255,0.06)', marginTop: 2 }}>
  ...
  return (
    <div style={{ padding: '12px 16px', borderRadius: 12, background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.07)' }}>
      ...
      <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        {lk.status === 'idle' ? (
          <button onClick={handleLookup} style={{ fontSize: 11, fontWeight: 600, cursor: 'pointer', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', borderRadius: 6, padding: '3px 10px', color: 'var(--color-muted)' }}>
            🔍 Look up result
          </button>
        ) : (
...
function ParlayBetRow({ bet, onUpdateBet, triggerCount = 0 }: {
  ...
  return (
    <div style={{ borderRadius: 12, overflow: 'hidden', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(129,140,248,0.18)' }}>
      ...
      <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap', borderBottom: '1px solid rgba(129,140,248,0.12)', background: 'rgba(129,140,248,0.05)' }}>
        ...
        {/* Settlement preview */}
        <div style={{ padding: '6px 12px', borderRadius: 8, background: settlement.settledCount > 0 ? (settlement.profit > 0 ? 'rgba(52,211,153,0.1)' : settlement.profit < 0 ? 'rgba(248,113,113,0.1)' : 'rgba(255,255,255,0.05)') : 'rgba(255,255,255,0.04)', border: `1px solid ${settlement.settledCount > 0 ? (settlement.profit > 0 ? 'rgba(52,211,153,0.25)' : settlement.profit < 0 ? 'rgba(248,113,113,0.25)' : 'rgba(255,255,255,0.1)') : 'rgba(255,255,255,0.08)'}`, textAlign: 'right', minWidth: 120 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 140 }}>
                  ...
                  <div style={{ fontSize: 11, color: 'var(--color-muted)', marginTop: 1 }}>
                    <span style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 3, padding: '1px 5px', marginRight: 5 }}>{leg.betType}</span>
                ...
                <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
                  {LEG_ORDER.map(r => {
                    ...
                    const active = cur === r
                    return (
                      <button key={r} onClick={() => handleLegResult(i, r)} title={m.label} style={{ padding: '5px 9px', borderRadius: 6, fontSize: 11, fontWeight: 700, cursor: 'pointer', background: active ? m.bg : 'rgba(255,255,255,0.04)', border: `1px solid ${active ? m.border : 'rgba(255,255,255,0.08)'}`, color: active ? m.color : 'var(--color-muted)', transition: 'all 0.15s' }}>
                        {m.short}
                      </button>
                    )
                  })}
      ...
      <div style={{ padding: '8px 12px 10px', display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 8, borderTop: dirty ? '1px solid rgba(251,191,36,0.15)' : '1px solid transparent', transition: 'border-color 0.2s' }}>
        {dirty && (
          <button onClick={handleReset} style={{ padding: '6px 14px', borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: 'pointer', background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--color-muted)' }}>Reset</button>
        )}
        <button onClick={handleSave} disabled={!dirty} style={{ padding: '7px 20px', borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: dirty ? 'pointer' : 'default', background: dirty ? 'rgba(129,140,248,0.2)' : 'rgba(255,255,255,0.04)', border: `1px solid ${dirty ? 'rgba(129,140,248,0.5)' : 'rgba(255,255,255,0.08)'}`, color: dirty ? 'var(--color-accent)' : 'var(--color-muted)', transition: 'all 0.2s' }}>
