      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <select value={sortBy} onChange={e => setSortBy(e.target.value as typeof sortBy)}
            style={{ padding: '5px 10px', borderRadius: 8, fontSize: 12, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.08)', color: 'var(--color-muted)', cursor: 'pointer', outline: 'none' }}>
              ...
              {filtered.map(bet => {
                ...
                return (
                  <React.Fragment key={bet.id}>
                    <tr className={`row-${bet.result}`} style={{ borderBottom: isExpanded ? 'none' : '1px solid rgba(255,255,255,0.04)', transition: 'background 0.15s' }}
                      ...
                      <td style={{ padding: '12px', textAlign: 'right' }}>
                        <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end' }}>
                          ...
                          <button onClick={() => handleDelete(bet.id)} style={{
                            padding: '4px 10px', borderRadius: 6, fontSize: 11, cursor: 'pointer', fontWeight: 600,
                            background: confirmDelete === bet.id ? 'rgba(248,113,113,0.2)' : 'rgba(255,255,255,0.05)',
                            border: confirmDelete === bet.id ? '1px solid rgba(248,113,113,0.4)' : '1px solid rgba(255,255,255,0.08)',
                        ...
                        <td colSpan={10} style={{ padding: '0 12px 14px 36px' }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 7, borderLeft: '2px solid rgba(129,140,248,0.2)', paddingLeft: 12 }}>
                            {bet.legs!.map((leg, i) => {
                              ...
                              return (
                                <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px 10px', borderRadius: 8, background: 'rgba(255,255,255,0.03)', border: `1px solid ${meta.border}` }}>
                                  {/* Leg info */}
                                  <div style={{ minWidth: 0, flex: 1 }}>
                                    ...
                                    <div style={{ fontSize: 11, color: 'var(--color-muted)', marginTop: 2 }}>
                                      <span style={{ background: 'rgba(255,255,255,0.06)', borderRadius: 4, padding: '1px 5px', marginRight: 5 }}>{leg.betType}</span>
