...
import { DailyPnL } from '@/hooks/useBets'
import { fmtPnL, fmtK } from '@/utils/currency'

interface Props { data: DailyPnL[] }

function CustomTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload: DailyPnL }> }) {
  if (!active || !payload?.length) return null
  const d = payload[0].payload
  const color = d.profit >= 0 ? 'var(--color-win)' : 'var(--color-loss)'
  return (
    <div style={{ background: '#0e1a0e', border: '1px solid rgba(240,235,224,0.12)', borderRadius: 10, padding: '10px 14px', fontSize: 13 }}>
  ...
  if (data.length === 0) {
    return (
      <div className="glass-card" style={{ padding: 24, height: 280, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
        <span style={{ color: 'var(--color-muted)' }}>No settled bets yet</span>
      </div>
    )
  }
  ...
  return (
    <div className="glass-card" style={{ padding: 24 }}>
      ...
      <ResponsiveContainer width="100%" height={220} minWidth={0}>
        <BarChart data={data} margin={{ top: 4, right: 8, left: 0, bottom: 4 }} barSize={data.length > 20 ? 8 : data.length > 10 ? 14 : 24}>
